...
import mamani.luna.diccionariosistemas.databinding.MeaningRecyclerRowBinding

class MeaningAdapter(private var meaningList : List<Meaning>) : RecyclerView.Adapter<MeaningAdapter.MeaningViewerHolder>(){

    class MeaningViewerHolder(private val binding: MeaningRecyclerRowBinding) : RecyclerView.ViewHolder(binding.root){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MeaningViewerHolder {
        val binding = MeaningRecyclerRowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MeaningViewerHolder(binding)
    }
    ...
    override fun onBindViewHolder(holder: MeaningViewerHolder, position: Int) {
